...
import org.mumod.util.MustardUtil;
	...
	private class VersionTask extends  AsyncTask<Void, Void, RetrieveResult> {
		...
		public RetrieveResult doInBackground(Void... params) {
			...
			try {
				...
				if(mSendSnapshot) {
					try {
						MustardUtil.snapshot(getBaseContext(), mid, MustardApplication.sVersionName, String.valueOf(c.getCount()));
					} catch (Exception e) {
				...
				while(c.moveToNext()) {
					...
					if (instance!=null && !"".equals(instance)) {
						...
						mDbHelper.setAttachmentTextLimitInstance(instance, configuration.site.textlimit, configuration.site.attachmentMaxSize);
